			...
			this.removeMarker('poiLocations')
				.then(() => {
					...
					this.map.on('click', 'poiLocations', (e) => {
						...
						new MapBox.Popup().setLngLat(coordinate).setHTML(`<p>${description}</p><br><a href='#/seek-help/${this.$store.state.pages.pageData.id}/detail/${id}'>Detail</a>`).addTo(this.map)
					});
